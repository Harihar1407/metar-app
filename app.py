    ...
    try:
        ...
        if response.status_code != 200:
            ...
            if not metar:
                ...
                pressure = f"{data.get('altim', 'N/A')} hHg"
                ...
                **Altimeter:** {pressure}
                ...
                with col1:
                    ...
                    direction = wind_direction(data.get("wdir"))

                    st.metric(
                        "Wind Direction",
                        f"{direction} ({data.get('wdir', 'N/A')}°)"
                        )
                with col2:
                    st.metric("Visibility", f"{data.get('visib', 'N/A')} SM")
                    st.metric("Dew Point", f"{data.get('dewp', 'N/A')} °C")
                    flight = data.get("fltCat", "N/A")

                    if flight == "VFR":
                        st.success("🟢 Flight Category: VFR")
                    elif flight == "MVFR":
                        st.info("🔵 Flight Category: MVFR")
                    elif flight == "IFR":
                        st.warning("🟠 Flight Category: IFR")
                    elif flight == "LIFR":
                        st.error("🔴 Flight Category: LIFR")
                    else:
                        st.write(f"Flight Category: {flight}")
